...
## 面试模拟
### 1. 你好，请先简单介绍一下自己。
面试官您好，我叫孙赵龙，于2020年毕业于安徽工程大学软件工程专业，拥有 4 年的 Java 开发工作经验，对各种 Java 技术和框架具有较深的理解和实践经验。
### 2. 能详细说一下你在工作中参与过的主要项目吗？从项目的需求背景、技术选型以及你在其中承担的核心职责等方面展开。
好的。在 21年 - 24 年我在北京赛优职教育科技有限公司担任 Java 开发工程师期间，参与了多个关键项目。
以基于 SpringBoot 和 Dubbo 实现的大型 CRM 系统为例，这个项目的需求背景是公司业务发展需要一套高效、稳定和可靠的客户关系管理系统，同时实现跨系统数据同步。在技术选型上，我们采用了 SpringBoot 作为基础框架，Dubbo 用于实现分布式服务，Nacos 作为配置中心和注册中心。我在这个项目中的核心职责包括参与系统的 0 - 1 开发与设计，整合 Nacos 配置服务，确保各个服务能够正确获取配置信息并且稳定运行。
###### 另外，在 CRM 工作台的开发项目中，需求是为销售人员提供一个高效的工作平台，实现线索的有效管理和流转。我们使用了 SpringBoot、Dubbo 等框架，结合 Xxl - Job、IM 和 Redis 缓存等技术。我的主要工作是实现接口防抖功能以及线索流转规则核心功能。对于接口防抖，我通过分析业务场景和用户操作习惯，采用合适的技术手段，在不影响用户体验的前提下，有效避免了接口的重复调用。在线索流转规则方面，我深入了解业务逻辑，根据不同的线索状态和时间节点，设计了合理的流转规则，并通过 Redis 缓存提高了数据的读取效率，确保线索能够按照设定的规则（如打电话、加企微、推班、回访等指标数据，以及当、三、七及公海流失规则）进行定时流转。
在电销外呼系统项目中，由于电销是公司 CRM 系统的核心业务需求，所以该系统需要具备稳定、高效、可靠以及高拓展性，以满足公司各业务产品的调用需求。我们参考优秀开源框架 Jeepay 进行设计开发，采用策略模式提高拓展性，数据库使用 MyBatisPlus 框架。我负责整个系统的设计开发工作，在项目中引入设计模式解决渠道快速迭代问题，通过深入分析不同渠道的特点和业务需求，设计了一套灵活的架构模式，使得系统能够快速适应渠道的变化。对于录音文件存档缓慢的问题，我选择 RabbitMQ 作为消息中间件，通过合理设计消息队列的结构和处理流程，实现了录音文件的高效存档，确保系统的稳定性和数据的完整性。同时，在话单存储方面，结合 MySQL 分表技术，有效提高了话单存储和查询的效率,其次，为了解决质检困难和话术优化问题，我们又引入了ASR技术，通过语音识别和大模型学习的方式解决这些问题。
###### 还有公司支付中心项目，该项目需要确保用户认证和授权的安全性和可靠性。我们使用 SpringSecurity 实现用户认证和授权，我负责实现关键模块及业务逻辑。通过研究 SpringSecurity 的认证和授权机制，根据公司的业务需求，定制了一套完善的用户权限管理体系，确保不同用户角色能够在安全的前提下，顺利访问和操作相应的资源。
