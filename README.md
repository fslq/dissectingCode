...
## 面试模拟
### 1. 你好，请先简单介绍一下自己。
面试官您好，我叫孙赵龙，安徽淮南人，于2020年毕业于安徽工程大学软件工程专业，拥有 4 年的 Java 开发经验，到目前为止就职过两家公司。
### 2. 能详细说一下你在工作中参与过的主要项目吗？从项目的需求背景、技术选型以及你在其中承担的核心职责等方面展开。
###### 20年 - 21 年,我在昆山中创软件担任 Java 开发工程师，参与了金融风险管理系统和数据报表的开发项目。风险管理系统是用来负责实时监控和分析风险数据的，数据报表项目则是用来高效准确地生成各种报表（比如用户贷款相关）。技术方案上：使用的是 SpringCloud 和存储过程技术。在实现数据报表项目中，通过多线程和并发编程技术解决报表批量上传性能缓慢问题（分片）。其间，我还负责了部分前端 Vue 项目的开发。
在 21年 - 24 年我在北京赛优职教育科技有限公司 担任 Java 开发工程师，参与了多个关键项目。
主要以 CRM 系统为例，这个项目的需求背景是：公司业务发展需要一套高效、稳定、可靠的客户关系管理系统，同时需要实现与第三方CRM系统的数据迁移。在技术选型上，我们采用的基础框架是 SpringBoot ，分布式服务用的是Dubbo ，配置中心和注册中心用的是Nacos 。我参与了整个系统的、从 0 - 1 的开发设计，主要包括：CRM 工作台和电销外呼系统。
###### CRM 工作台的业务需求是为销售人员提供一个高效,可靠的工作平台，功能上包括任务的分配、服务、流转和转化。技术实现上的是 定时任务、IM 和 Redis 缓存等技术。在线索流转方面，通过了解公司的业务逻辑，根据 不同的 线索状态 和 时间节点，设计合理的流转规则，并通过 缓存提高了数据的查询效率，确保线索能够按照设定的规则（如打电话、加企微、推班、回访等指标数据，以及当、三、七及公海流失规则）进行定时流转。
在电销外呼系统项目中，担任组长一职，由于电销是公司 CRM 系统的核心业务，所以该系统需要具备 稳定性 和 拓展性。这里参考了优秀开源代码 Jeepay 的设计思路，采用策略模式提高拓展性，解决了渠道快速迭代问题。在处理录音文件存档缓慢的问题，我选择 RabbitMQ 作为消息中间件，通过合理设计消息结构和处理流程，实现了录音文件的高效存档，以保证系统的稳定性和数据的完整性。同时，在话单存储方面，结合 MySQL 分表技术，有效提高了话单存储和查询的效率,其次，引入了ASR技术解决了质检困难和话术优化问题，又引入了ASR技术，通过语音识别和大模型学习的方式解决这些问题。
